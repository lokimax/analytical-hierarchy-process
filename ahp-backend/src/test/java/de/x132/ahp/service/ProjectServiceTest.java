...
class ProjectServiceTest {
  ...
  void shouldFindAllByClient() {
    // Given
    Project project2 = Project.builder().id(2L).name("Project 2").client(testClient).build();

    List<Project> expectedProjects = Arrays.asList(testProject, project2);
    when(projectRepository.findAllByClient(testClient)).thenReturn(expectedProjects);
  ...
  void shouldFindByClientNicknameAndName() {
    ...
    Optional<Project> result =
        projectService.findByClientNicknameAndName("testuser", "Test Project");
  ...
  void shouldHandleNullClient() {
    // Given
    when(projectRepository.findAllByClient(null)).thenReturn(Arrays.asList());
