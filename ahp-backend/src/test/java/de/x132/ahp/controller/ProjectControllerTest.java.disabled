...
/**
 * Integration tests for ProjectController. Tests project creation, retrieval, update, and deletion.
 *
 * @author Max Wick
 */
@SpringBootTest
...
public class ProjectControllerTest {
  ...
  public void testCreateProject() throws Exception {
    Project newProject =
        Project.builder().name("Test Project").beschreibung("A test project for AHP").build();
  ...
  public void testCreateProjectWithoutAuthentication() throws Exception {
    Project newProject = Project.builder().name("Test Project").beschreibung("Should fail").build();
  ...
  public void testGetProjectByName() throws Exception {
    ...
    mockMvc
        .perform(get("/api/projects/TestProject").header("Authorization", "Bearer " + authToken))
        .andExpect(status().isOk())
  ...
  public void testGetNonexistentProject() throws Exception {
    mockMvc
        .perform(
            get("/api/projects/NonexistentProject").header("Authorization", "Bearer " + authToken))
  ...
  public void testDeleteProject() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/projects/ProjectToDelete").header("Authorization", "Bearer " + authToken))
        .andExpect(status().isNotFound());
  }
  ...
  public void testCreateProjectRoundTrip() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/projects/" + createdProject.getName())
                .header("Authorization", "Bearer " + authToken))
        .andExpect(status().isOk())
    ...
    mockMvc
        .perform(
            put("/api/projects/" + createdProject.getId())
                .header("Authorization", "Bearer " + authToken)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    mockMvc
        .perform(
            delete("/api/projects/" + createdProject.getId())
                .header("Authorization", "Bearer " + authToken))
        .andExpect(status().isNoContent());
