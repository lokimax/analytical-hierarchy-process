 ...
 * Tests node and connection creation, retrieval, update, and deletion.
 *
 * @author Max Wick
 */
@SpringBootTest
...
public class NodeControllerTest {
    ...
    public void testCreateMultipleNodes() throws Exception {
        ...
        mockMvc.perform(get("/api/projects/TestProject/nodes")
                        .header("Authorization", "Bearer " + authToken))
                .andExpect(status().isOk())
    ...
    public void testGetNodeByName() throws Exception {
        ...
        mockMvc.perform(get("/api/projects/TestProject/nodes/SpecificNode")
                        .header("Authorization", "Bearer " + authToken))
                .andExpect(status().isOk())
    ...
    public void testCreateConnection() throws Exception {
        ...
        String connectionJson = String.format(
                "{\"sourceNodeName\": \"SourceNode\", \"targetNodeName\": \"TargetNode\"}");

        mockMvc.perform(post("/api/projects/TestProject/connections")
                        .header("Authorization", "Bearer " + authToken)
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    public void testCompleteNetworkCreation() throws Exception {
        ...
        for (int i = 1; i <= 3; i++) {
            String connectionJson = String.format(
                    "{\"sourceNodeName\": \"Goal\", \"targetNodeName\": \"Criterion%d\"}", i);
        ...
        for (int i = 1; i <= 3; i++) {
            for (int j = 1; j <= 2; j++) {
                String connectionJson = String.format(
                        "{\"sourceNodeName\": \"Criterion%d\", \"targetNodeName\": \"Alternative%d\"}", i, j);
    ...
    public void testCreateDuplicateNode() throws Exception {
        Node existingNode = Node.builder()
                .name("ExistingNode")
                .project(testProject)
                .build();
