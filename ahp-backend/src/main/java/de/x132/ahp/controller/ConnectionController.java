...
public class ConnectionController {
    ...
    public ResponseEntity<ConnectionResponse> createConnection(
        ...
        Project project = projectService.findAll().stream()
                .filter(p -> p.getName().equals(projectName))
                .findFirst()
                .orElseThrow(() -> new RuntimeException("Project not found"));

        // Find source and target nodes
        Node sourceNode = nodeService.findByProjectAndName(project, request.getSourceNodeName())
                .orElseThrow(() -> new RuntimeException("Source node not found"));
        
        Node targetNode = nodeService.findByProjectAndName(project, request.getTargetNodeName())
    ...
    @GetMapping
    public ResponseEntity<List<ConnectionResponse>> getAllConnections(@PathVariable String projectName) {
        Project project = projectService.findAll().stream()
                .filter(p -> p.getName().equals(projectName))
                .findFirst()
                .orElseThrow(() -> new RuntimeException("Project not found"));

        List<ConnectionResponse> connections = nodeService.findConnectionsByProject(project).stream()
