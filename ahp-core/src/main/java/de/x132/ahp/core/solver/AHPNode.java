...
/**
 * Represents a calculated AHP node using the Analytic Hierarchy Process. This class performs the
 * complete AHP calculation including: - Evolution matrix creation - Normalization - Eigenvector
 * calculation - Consistency checking (CR <= 0.1)
 *
 * @author Max Wick
 */
@Slf4j
...
public class AHPNode {
  ...
  private static final BigReal[] SAATY_RI =
      new BigReal[] {
  ...
  public AHPNode(Node node, Prioritisation prioritisation) {
    ...
    if (n > 2) {
      this.ci = lambdaMax.add(new BigReal(n).negate()).divide(new BigReal(n - 1));
   ...
   */
  private static FieldMatrix<BigReal> normalizeMatrixBy(
      FieldMatrix<BigReal> matrix, FieldVector<BigReal> columnSums) {
