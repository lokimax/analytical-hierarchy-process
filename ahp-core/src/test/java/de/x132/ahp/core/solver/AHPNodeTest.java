...
class AHPNodeTest {
    ...
    void shouldHandleTwoChildren() {
        ...
        BigDecimal sum = ahpNode.getSingleResults().stream()
    ...
    void shouldHandleExtremeWeights() {
        ...
        prioritisation.addComparison(createComparison(parent, critical, negligible, BigDecimal.valueOf(9)));
        ...
        SingleResult criticalResult = results.stream()
                .filter(r -> r.getNodeName().equals("Critical"))
                .findFirst()
                .orElseThrow();

        assertTrue(criticalResult.getValue().compareTo(new BigDecimal("0.8")) > 0,
    ...
    void shouldCalculatePrioritiesCorrectly() {
        ...
        SingleResult highResult = results.stream()
                .filter(r -> r.getNodeName().equals("High"))
                .findFirst().orElseThrow();
        SingleResult mediumResult = results.stream()
                .filter(r -> r.getNodeName().equals("Medium"))
                .findFirst().orElseThrow();
        SingleResult lowResult = results.stream()
                .filter(r -> r.getNodeName().equals("Low"))
                .findFirst().orElseThrow();

        // Verify High > Medium > Low
        assertTrue(highResult.getValue().compareTo(mediumResult.getValue()) > 0,
                "High should have higher priority than Medium");
        assertTrue(mediumResult.getValue().compareTo(lowResult.getValue()) > 0,
    ...
    void shouldCalculateLambdaMax() {
        ...
        assertTrue(lambdaMaxValue.subtract(BigDecimal.valueOf(ahpNode.getN()))
